...
const ANATOMY_SUGGESTIONS: LangMap<Record<string, string>> = {
  ...
  es: {
    role: 'Define un rol profesional: ej., "Actua como un desarrollador backend senior especializado en APIs REST"',
    task: 'Se especifico con tu objetivo: ej., "Crea un endpoint REST API que valide la entrada del usuario y devuelva una respuesta JSON estructurada"',
    context: 'Proporciona contexto: stack tecnologico, tipo de proyecto, entorno y audiencia objetivo',
    steps: 'Desglosa la tarea en pasos: ej., "Primero, analiza los requisitos. Luego, implementa la solucion. Finalmente, escribe tests."',
    format: 'Especifica el formato de salida esperado: ej., "Devuelve como JSON", "Formatea como tabla markdown", "Proporciona como bloque de codigo"',
    constraints: 'Anade restricciones: ej., "Evita APIs obsoletas", "Maximo 200 lineas", "Sigue los principios SOLID"',
    clarification: 'Anade una clausula de clarificacion: ej., "Si algo no esta claro, pregunta antes de continuar"',
  },
};
...
const ISSUE_SUGGESTIONS: LangMap<Record<string, string>> = {
  ...
  es: {
    vague_instruction: "Anade detalles mas especificos sobre lo que quieres lograr",
    missing_context: "Proporciona informacion de contexto o antecedentes para mejores resultados",
    no_output_format: 'Especifica el formato de salida deseado (ej., "Responde en formato JSON")',
    missing_role: 'Define un rol para la IA (ej., "Actua como un desarrollador senior...")',
    too_long: "Considera dividir esto en varios prompts mas pequenos y enfocados",
    redundant: "Elimina el contenido repetitivo para hacer el prompt mas conciso",
    vague_terms: 'Reemplaza terminos vagos como "algo" o "cosas" con sustantivos especificos',
    no_constraints: 'Anade restricciones para acotar la salida (ej., "Evita usar APIs obsoletas")',
    no_success_criteria: 'Define criterios de exito (ej., "El resultado debe manejar casos limite")',
    no_audience: 'Especifica la audiencia objetivo (ej., "para desarrolladores principiantes")',
    missing_examples: "Anade ejemplos de la entrada y salida esperada (Few-Shot prompting)",
    ...
    poor_structure: "Mejora la estructura del prompt usando parrafos, encabezados y viñetas",
    virtualization_risk: 'Anade restricciones estrictas (ej., "Manten solo el personaje") al usar virtualizacion o simulacion',
    payload_splitting_risk: "Revisa si hay espaciado o codificacion sospechosa que pueda interpretarse como un intento de inyeccion",
    security: "Revisa y elimina cualquier contenido que pueda interpretarse como inyeccion de prompt",
    positive: "Tu prompt se ve bien. Considera anadir ejemplos para obtener resultados aun mejores.",
  },
};
