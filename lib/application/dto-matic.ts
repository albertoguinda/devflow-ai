...
export function generateCode(
  ...
  switch (config.targetLanguage) {
    ...
    case "python":
      ...
      break;
    case "sql":
      files.push(...generateSQL(parsed.fields, rootName, config));
      break;
    case "typescript":
    default:
...
function generateSQL(
  ...
  const dialect = config.sqlDialect ?? "postgresql";
  const tableName = sqlSnakeCase(rootName);
  const lines: string[] = [];
  const nestedTables: string[] = [];

  lines.push(`-- Generated by DTO-Matic (${dialect.toUpperCase()})`);
  lines.push("");
  lines.push(`CREATE TABLE ${tableName} (`);
  lines.push(`  id ${dialect === "postgresql" ? "SERIAL" : "INT AUTO_INCREMENT"} PRIMARY KEY,`);

  for (let i = 0; i < fields.length; i++) {
    const field = fields[i]!;
    const colName = sqlSnakeCase(field.name);
    const sqlType = mapFieldToSqlType(field, dialect);
    const nullable = field.isOptional ? "" : " NOT NULL";
    const isLast = i === fields.length - 1;

    if (field.type === "object" && field.children) {
      const nestedTable = sqlSnakeCase(field.name);
      lines.push(`  ${colName}_id INTEGER${nullable} REFERENCES ${nestedTable}(id)${isLast ? "" : ","}`);
      nestedTables.push(generateNestedSqlTable(field.children, nestedTable, dialect));
    } else {
      lines.push(`  ${colName} ${sqlType}${nullable}${isLast ? "" : ","}`);
    }
  }

  lines.push(");");

  const content = [...nestedTables, lines.join("\n")].join("\n\n");
...
function generateNestedSqlTable(
  fields: JsonField[],
  tableName: string,
  dialect: string
): string {
...
function mapFieldToSqlType(field: JsonField, dialect: string): string {
  if (field.isDate) return "TIMESTAMP";
  if (field.semanticType === "uuid") return "UUID";
  if (field.semanticType === "email" || field.semanticType === "url") return "VARCHAR(255)";
  if (field.semanticType === "ipv4") return dialect === "postgresql" ? "INET" : "VARCHAR(45)";

  switch (field.type) {
    case "string": return "VARCHAR(255)";
    case "number": return field.isFloat ? "DECIMAL(10,2)" : "INTEGER";
...
function sqlSnakeCase(name: string): string {
  return name
    .replace(/([A-Z])/g, "_$1")
    .toLowerCase()
    .replace(/^_/, "")
...
function generateInterface(
  ...
  if (isRoot) {
    ...
    lines.push("// https://devflowai.dev/tools/dto-matic");
    lines.push("");
  }
