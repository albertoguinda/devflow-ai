...
const WIZARD_STRINGS = {
  ...
  fr: {
    reasoning: {
      recommendedConvention: (type: string) => `Convention recommandée pour ${type}`,
      optimalLength: "Longueur optimale",
      reactHookPattern: "Suit le modèle des hooks React",
      descriptiveActionPrefix: "Préfixe d'action descriptif",
      standardConstantConvention: "Convention standard pour les constantes",
      descriptiveName: "Nom descriptif",
    },
    audit: {
      tooGeneric: (word: string) => `Nom trop générique : "${word}" ne décrit pas le contenu.`,
      tooShort: "Nom trop court, peut manquer de clarté.",
      tooLong: "Nom excessivement long, envisagez de simplifier.",
      booleanPrefix: "Les états booléens devraient commencer par un verbe (is, has, can...).",
      avoidNumbers: "Évitez les nombres dans les noms sauf si strictement nécessaire.",
      hungarianNotation: "Évitez la notation hongroise (strName, intCount) ; le type est déjà défini par le langage.",
    },
  },
  pt: {
    reasoning: {
      recommendedConvention: (type: string) => `Convenção recomendada para ${type}`,
      optimalLength: "Comprimento ideal",
      reactHookPattern: "Segue o padrão de hooks do React",
      descriptiveActionPrefix: "Prefixo de ação descritivo",
      standardConstantConvention: "Convenção padrão para constantes",
      descriptiveName: "Nome descritivo",
    },
    audit: {
      tooGeneric: (word: string) => `Nome muito genérico: "${word}" não descreve o conteúdo.`,
      tooShort: "Nome muito curto, pode ser pouco claro.",
      tooLong: "Nome excessivamente longo, considere simplificar.",
      booleanPrefix: "Estados booleanos devem começar com um verbo (is, has, can...).",
      avoidNumbers: "Evite números nos nomes a menos que seja estritamente necessário.",
      hungarianNotation: "Evite a notação húngara (strName, intCount); o tipo já é definido pela linguagem.",
    },
  },
  de: {
    reasoning: {
      recommendedConvention: (type: string) => `Empfohlene Konvention für ${type}`,
      optimalLength: "Optimale Länge",
      reactHookPattern: "Folgt dem React-Hooks-Muster",
      descriptiveActionPrefix: "Beschreibendes Aktionspräfix",
      standardConstantConvention: "Standardkonvention für Konstanten",
      descriptiveName: "Beschreibender Name",
    },
    audit: {
      tooGeneric: (word: string) => `Zu generischer Name: „${word}" beschreibt den Inhalt nicht.`,
      tooShort: "Name zu kurz, kann unklar sein.",
      tooLong: "Übermäßig langer Name, erwägen Sie eine Vereinfachung.",
      booleanPrefix: "Boolesche Zustände sollten mit einem Verb beginnen (is, has, can...).",
      avoidNumbers: "Vermeiden Sie Zahlen in Namen, es sei denn, es ist unbedingt erforderlich.",
      hungarianNotation: "Vermeiden Sie die ungarische Notation (strName, intCount); der Typ wird bereits durch die Sprache definiert.",
    },
  },
  it: {
    reasoning: {
      recommendedConvention: (type: string) => `Convenzione consigliata per ${type}`,
      optimalLength: "Lunghezza ottimale",
      reactHookPattern: "Segue il pattern degli hooks di React",
      descriptiveActionPrefix: "Prefisso di azione descrittivo",
      standardConstantConvention: "Convenzione standard per le costanti",
      descriptiveName: "Nome descrittivo",
    },
    audit: {
      tooGeneric: (word: string) => `Nome troppo generico: "${word}" non descrive il contenuto.`,
      tooShort: "Nome troppo corto, potrebbe essere poco chiaro.",
      tooLong: "Nome eccessivamente lungo, considera di semplificare.",
      booleanPrefix: "Gli stati booleani dovrebbero iniziare con un verbo (is, has, can...).",
      avoidNumbers: "Evita i numeri nei nomi a meno che non sia strettamente necessario.",
      hungarianNotation: "Evita la notazione ungherese (strName, intCount); il tipo è già definito dal linguaggio.",
    },
  },
  zh: {
    reasoning: {
      recommendedConvention: (type: string) => `${type} 的推荐命名约定`,
      optimalLength: "最佳长度",
      reactHookPattern: "遵循 React hooks 模式",
      descriptiveActionPrefix: "描述性动作前缀",
      standardConstantConvention: "常量的标准命名约定",
      descriptiveName: "描述性名称",
    },
    audit: {
      tooGeneric: (word: string) => `名称过于通用："${word}" 未能描述内容。`,
      tooShort: "名称过短，可能含义不清。",
      tooLong: "名称过长，建议简化。",
      booleanPrefix: "布尔状态应以动词开头（is、has、can...）。",
      avoidNumbers: "除非确有必要，否则请避免在名称中使用数字。",
      hungarianNotation: "请避免匈牙利命名法（strName、intCount）；类型已由语言本身定义。",
    },
  },
  ja: {
    reasoning: {
      recommendedConvention: (type: string) => `${type} の推奨命名規則`,
      optimalLength: "最適な長さ",
      reactHookPattern: "React hooks パターンに準拠",
      descriptiveActionPrefix: "説明的なアクションプレフィックス",
      standardConstantConvention: "定数の標準命名規則",
      descriptiveName: "説明的な名前",
    },
    audit: {
      tooGeneric: (word: string) => `汎用的すぎる名前：「${word}」は内容を説明していません。`,
      tooShort: "名前が短すぎて、不明確になる可能性があります。",
      tooLong: "名前が長すぎます。簡略化を検討してください。",
      booleanPrefix: "ブール状態は動詞で始めるべきです（is、has、can...）。",
      avoidNumbers: "厳密に必要な場合を除き、名前に数字を使用しないでください。",
      hungarianNotation: "ハンガリアン記法（strName、intCount）は避けてください。型は言語によって既に定義されています。",
    },
  },
} as const;
